...
export default function List() {
  ...
  useEffect(() => {
    if (["create","update","delete","error"].includes(isMemberCreate)) {
      let msg = "", alertIcon = "";
      switch (isMemberCreate) {
        case "create": msg="Member Created Successfully!"; alertIcon="success"; break;
        case "update": msg="Member Updated Successfully!"; alertIcon="success"; break;
        case "delete": msg="Member Deleted Successfully!"; alertIcon="success"; break;
        case "error": msg="Some internal error!"; alertIcon="error"; break;
      }
                    ...
                    <td className="px-4 py-2">
                      <div className="flex items-center justify-center">
                        <div className="w-16 h-16 rounded-full overflow-hidden flex items-center justify-center">
                          {member.mem_img && !imgErrors[member.id] ? (
                            <img
                              src={member.mem_img}
                              alt={member.mem_name}
                            ...
                            <div
                              className={`w-full h-full flex items-center justify-center text-white text-xl font-semibold ${getBgColor(
                                member.mem_name
                              )}`}
                            >
                              {getInitials(member.mem_name)}
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    {imagePreview && (
                      <div className="flex flex-col mt-2">
                        ...
                        <div className="w-full h-40 border border-gray-700 rounded-lg bg-gray-800 flex items-center justify-center p-2">
                          {imgPreviewError ? (
                            <div
                              className={`w-32 h-32 rounded-full flex items-center justify-center text-white text-2xl font-semibold ${getBgColor(
                                watch("mem_name")
                              )}`}
                            >
                              {getInitials(watch("mem_name"))}
                            </div>
                          ) : (
                            <img
                              src={imagePreview}
